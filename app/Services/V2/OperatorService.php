...
class OperatorService {
     ...
     * @throws HttpException
     */
    public function listByMemberType(ListQueryParam $param, MemberType|null $type, CountryParam $country) : PageCollection {
        if($this->user) {
            $query = User::query()
                ->select('users.*')
                ->orderBy($param->order, $param->direction)
                ->when($type, function(Builder $query) use($type) {
                    if($type == MemberType::TYPE_ALL) return;
                    $query->join('user_types', 'users.id', '=', 'user_types.user_id')
                        ->where('user_types.type', $type->value);
                })

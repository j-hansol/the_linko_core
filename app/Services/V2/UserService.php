...
class UserService {
    ...
    public function joinPerson(
        ...
        if(!$this->user) {
            ...
            $user = User::create($common->toArray() + $person->toArray() + $auth->toArray() + $t);
    ...
    public function joinOrganization(
        ...
        if(!$this->user) {
            ...
            $user = User::create($common->toArray() + $organization->toArray() + $auth->toArray() + $t);
            if($auth->getHashedPassword()) PasswordHistory::createByUser($user, $auth->getHashedPassword());
            UserType::createType($user, $common->getType());
    ...
    public function login(PasswordLoginDto $dto) : UserTokenDto {
        ...
        else throw HttpErrorsException::getInstance([__('errors.user.already_logged_in')], 406);
    }

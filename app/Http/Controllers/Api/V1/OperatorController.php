...
use App\Http\Requests\V1\RequestAddVisaDocumentType;
use App\Http\Requests\V1\RequestEvalInfo;
use App\Http\Requests\V1\RequestEvalItem;
use App\Http\Requests\V1\RequestUpdateOccupationalGroup;
use App\Http\Requests\V1\RequestUpdateUserActiveState;
...
use App\Services\V1\OperatorService;
...
class OperatorController extends Controller {
     ...
     * )
     */
    public function listUser(ListQueryParam $param, UserTypeParam $param2) : JsonResponse {
        try {
            $service = OperatorService::getInstance();
            if(!$param2->type) throw new \Exception('user type required.');

            return new UserInfos(
                new PageResult(
                    $service->listByMemberType($param, $param2->type), $param
                )
            );
        } catch (HttpException $e) {
            return new Message($e->getCode());
